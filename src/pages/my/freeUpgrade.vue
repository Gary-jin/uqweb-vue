<template>
    <div class="freeUpgrade" ref="freeUpgrade" v-if="data" :style="{overflowY: dialog ? 'hidden' : 'auto'}">
      ...
      >
        <a @click="goBack()" v-if="$isShowBackBtn" slot="left" class="backBox"><i class="iconfont icon-back"></i></a>
        {{title}}
      </x-header>
      <div class="upgradeBox" ref="upgradeBox">
        ...
        <div class="status">
          ...
          <div v-if="data.is_show.is_team === '1'">
            ...
            <div class="invite" @click="goNativeHome()">去下单</div>
          </div>
...
export default {
  ...
  created () {
    // console.log('isShowBackBtn', $isShowBackBtn)
    if (this.$bizId === 4) {
      this.title = '运营总监'
