<template>
  <div id="Video" class="videoIndex">
    <x-header
      ...
      <a v-if="$isShowBackBtn" @click="goBack()" slot="left" class="backBox"><i class="iconfont icon-back"></i></a>
      共享影视
      <p @click="showRule" slot="right" style="color:#333">规则</p>
    </x-header>
    ...
    <div v-transfer-dom>
      <x-dialog v-model="show1">
        <h4 style="padding-top:0.2rem">观影规则</h4>
        <div style="padding:0.3rem 0.2rem;height:4rem;overflow-y:auto;text-align:left;" v-html="data.rule"></div>
...
export default {
  ...
  components: {
    swiper, swiperSlide, Confirm, XDialog
  },
  data () {
    return {
      search: '',
  ...
  methods: {
    ...
    * */
    ...
    getVideoInfo () {
      ...
      this.$http.get('/amoy/share-video/video-list', {page: 0, type: 5}, true, false).then(res => {
        if (res.code == 0) {
    ...
    watchVideo (item, watchType) {
      if (this.power === 1) {
        this.jumpTo('/videoDetail?url=' + item.path + '&title=' + item.title)
