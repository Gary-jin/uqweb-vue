  ...
  mounted() {
    ...
    this.$http.post('/amoy/user/user-new', {}, true, true).then(res => {
      if (res.code === 0) {
        ...
        if(dsBridge.call("receiveParams", 'avatar') === '' || dsBridge.call("receiveParams", 'avatar') === null){
          _this.avatar = res.data.logo;
        }else{
          _this.avatar = dsBridge.call("receiveParams", 'avatar')
        }
      }
    });
    this.moblie = dsBridge.call("receiveParams", 'mobile')
  },
  methods:{
