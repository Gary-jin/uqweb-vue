<template>
  <div id="classify">
    ...
    <!-- <div class="header" :style="{paddingTop: statusH + 'rem',color: $store.state.global.search.color,background: 'linear-gradient('+$store.state.global.search.bgDeg+'deg,' +$store.state.global.search.bgColor+','+$store.state.global.search.bgSecondColor+')'}">
      <i class="iconfont icon-back" v-if="$isShowBackBtn" @click="goBack()"></i>
      ...
      <span @click="jumpTo('/search')">搜索</span>
    </div> -->
    <div class="headerConBox">
      <div class="header">
        <i class="iconfont icon-back" @click="goBack()"></i>
        <p class="title">{{this.$route.query.title}}</p>
      </div>
    </div>
...
<script>
import {Tab, TabItem, Swiper, SwiperItem, Sticky, Popup} from 'vux'
  ...
  methods: {
    navHttp (item) {
      if (item.url) {
        ...
        dsBridge.call('open', {url: url})
      }
    },
    openVideo (item) {
      this.title = item.title.slice(0, 7) + '...'
      this.videoUrl = item.video_url
      // this.isShowVideo = true
      this.jumpTo('/classifyVideo?url=' + item.video_url + '&title=' + this.title)
      // dsBridge.call("openVideo", {title: title, url: item.video_url})
    },
