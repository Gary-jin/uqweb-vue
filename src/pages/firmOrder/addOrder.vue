<template>
  <div id="add-site">
    ...
    >
      <a @click="vueGoBack()" slot="left" class="backBox"><i class="iconfont icon-back"></i></a>
      添加地址
    </x-header>
    <div class="site-body">
      <div class="details">
      ...
      <div class="confirm" @click="submit">
        <button style="borderRadius:0.1rem" :style="{background: 'linear-gradient('+$store.state.global.theme.deg+'deg,' +$store.state.global.theme.mainColor+','+$store.state.global.theme.secondColor+')'}">确定</button>
      </div>
    </div>
  </div>
</template>
