  ...
  <div id="homeFirmOrder" class="homeFirmOrder" >
    ...
    <div class="orderBox">
      ...
      <div class="noOrderBox" v-if="!addressList">
        ...
        <p class="addOrderBtn">
          <span class="addBtnText" @click="vueJumpTo('/addOrder')">添加收货地址</span>
    ...
    <div class="yesBtn" @click="goShopp()" :style="{background: 'linear-gradient('+$store.state.global.theme.deg+'deg,' +$store.state.global.theme.mainColor+','+$store.state.global.theme.secondColor+')'}">确认</div>
  </div>
</template>
...
export default {
  ...
  methods: {
    getInfo () {
      var that = this
      this.$http.get('/amoy/user/upgrade-goods', {id: that.id}, true, true).then(res => {
        if (res.code == 0) {
    ...
    getCardCheckout () {
      var that = this
      this.$http.post('/amoy/user/address-list', {}, true, true).then(res => {
        if (res.code == 0) {
