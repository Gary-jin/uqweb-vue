      ...
      <div class="mescrollBox" :style="{'bottom': showFoot ? '50px' : '0'}">
        <mescroll-vue ref="mescroll" :down="mescrollDown" :up="mescrollUp" @init="mescrollInit">
          <div>
            ...
            <div v-if="!list" class="cart-null" style="background:transparent">
...
export default {
  ...
  methods: {
    getIndex () {
      apiHttp.goodLickList(this.$codeVersion).then(response => {
        if (response.code === 0) {
    ...
    del () {
      ...
      apiHttp.goodsLikeDelete(str, this.$codeVersion).then(response => {
        if (response.code === 0) {
