<template>
  <div id="exchange">
    ...
    >
      <a @click="toBack" slot="left" class="backBox"><i class="iconfont icon-back"></i></a>
      退款申请
    </x-header>
...
export default {
  ...
  methods: {
    // 返回可实现数据刷新
    toBack(){
      this.vueJumpTo('/mall/fixing_order?id=2')
    },
    // 删除上传的图片
    deleteImg (i) {
    ...
    submit () {
      ...
      apiHttp.submitInfo('save', '', this.obj.orderSn, this.text + ',' + this.info, this.allMary, this.text1.id, arr, this.phones, this.express, this.expressNo).then(response => {
        if (response.code === 0) {
          ...
          // 退款成功返回订单列表退换货列表，实现数据刷新。
          setTimeout(()=>{
            this.vueJumpTo('/mall/fixing_order?id=5')
          },1000)
        }
      })
    },
