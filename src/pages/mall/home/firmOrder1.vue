<template>
    <div class="firmOrder_main" id="apply">
        <x-header
          slot="header"
        ...
        <div v-for="(items,index) in obj.cartList" :key="index" class="one-shop-order">
          ...
          <div class="content_main" v-if="items.shopInfo.deduce" style="margin-top:0.1rem;">
            <div class="list jifen-box">
              ...
              <input type="number" placeholder="请输入积分数量" v-model="items.inputJifenNum" @blur="changeJifen(items, items.inputJifenNum)">
            </div>
          </div>
          <div v-if="items.shopInfo.deduce.type==3" style="height: 4rem;"></div>
        </div>
      </div>
      <div class="footer">
          <div class="footer_main">
            <div class="div1">
              <label>合计</label><span>¥</span>{{sumPrice.toFixed(2)}}
            </div>
...
export default {
  ...
  computed: {
      ...
      get () {
        ...
        for (let i = 0, len = this.obj.cartList.length; i < len; i++) {
          if (this.obj.cartList[i].useJifen) {
            ...
            console.log('userJifenNum', this.obj.cartList[i].useJifenNum)
    ...
    sumPrice () {
      ...
      for (let i = 0, len = this.obj.cartList.length; i < len; i++) {
        if (this.obj.cartList[i].useJifen) {
          if (this.obj.cartList[i].shopInfo.deduce.type == '3') {
            sumPrice = sumPrice - this.obj.cartList[i].useJifenNum / this.jifenRate
          } else {
            sumPrice = sumPrice - this.obj.cartList[i].useJifenNum
          }
        }
      }
      return sumPrice
  ...
  methods: {
    ...
    changeJifen (items, val) {
      ...
      if (items.shopInfo.deduce.type == 3) {
        ...
        console.log('this.obj.credit1', this.obj.credit1)
        if (val && +this.obj.credit1 < +items.shopInfo.deduce.credit1) {
          // 当要开启此积分抵扣但是可用积分不足时
          items.useJifen = 0
