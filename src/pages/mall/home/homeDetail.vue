<template>
  <div class="home_detail" style="height:100%;">
      ...
      <div class="detail_main" id="detail_main" ref="my_scroller">
          ...
          <div class="img">
            <swiper v-if="detailObj.thumbs" :options="swiperOption" ref="mySwiper" style="height:100%;">
              <swiperSlide v-for="(item,index) in detailObj.thumbs" :key="index">
                <img v-lazy="item" alt="" style="width:100%;">
              </swiperSlide>
            </swiper>
        ...
        <div class="productReview" id="productReview">
          <div class="title">
            ...
            <div class="right" @click="jumpTo('/mall/comment?id=' + detailObj.id)">
    ...
    <div class="footer" :style="{paddingBottom: bot + 'rem',height:1.1 + bot + 'rem'}">
      <div class="left">
        <div class="div_flex" @click="jumpTo('/mall/home')">
    ...
    <div v-transfer-dom>
      <popup v-model="show_select" position="bottom"  width="100%">
        <div class="detail_model">
            ...
            <div class="num_main">
              <x-number title="购买数量" v-model="changeValue" width="0.8rem" :min="1" button-style="round"></x-number>
            </div>
...
export default {
  ...
  mounted: function () {
    ...
    apiHttp.goodsDetail(this.$route.query.title, this.$codeVersion).then(response => {
      if (response.code === 0) {
  ...
  methods: {
    ...
    handService () {
      if (this.shopInfo.bdsqUrl) {
        this.jumpTo('/mall/CSD?url=' + encodeURIComponent(this.shopInfo.bdsqUrl))
    ...
    // 收藏商品
    handShou () {
      apiHttp.goodsLike(this.$route.query.title, this.$codeVersion).then(response => {
        if (response.code === 0) {
          if (this.isLick === 0) {
            this.isLick = 1
    ...
    handShangGo (key) {
      this.jumpTo('/mall/seller?spid=' + this.detailObj.spid + '&type=' + key)
      // this.$router.push({
      //   name: 'mallSeller',
      //   query: {
    ...
    * */
    send (key) {
      ...
      if (key === 1) {
        ...
        apiHttp.cardInterim(this.$route.query.title, goodsNum, this.attrId).then(response => {
          if (response.code === 0) {
            this.jumpTo('/mall/firmOrder?data=' + response.data)
          }
        })
      }
    },
    shareFri (type) {
      ...
      let that = this
      if (that.userPhone === '' || that.userPhone === null && this.$codeVersion >= 300){
      ...
      dsBridge.call("inviteShare", info, function(ret){
        if (ret === 'success') {
          that.$http.post('/mall/users/share', {goodsId: this.$route.query.id}).then(res => {
            if (res.code === 0) {
              // alert('1')
            } else {
              // alert('error')
