  ...
  <div class="payUnder">
    ...
    <div class="payUnder_content">
      ...
      <div class="input_box">
        ...
        <div class="right">
          <input type="number" placeholder="请输入支付金额" v-model="price" @blur="onBlur" @input="oninput">
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    handleBuy (id) {
      ...
      if (this.price <= 0){
        this.$vux.toast.text('金额有误，请重新输入')
        return
      }
      apiHttp.underPayInfo('save', id, this.price, deduce).then(res => {
        if (res.code === 0) {
          this.jumpTo('/mall/UnderpayMent?spidInfo=' + res.data + '&spid=' + this.$route.query.spid)
        }
      })
    }
  }
