<template>
  <div class="hy-recharge">
    <x-header :left-options="{showBack: false}" class="collection-head" >
      ...
      <img @click="jumpTo('/mall/rechargeList?spid='+spid)" class="recharge-img" src="../../../assets/mall/img/under/czList.png" alt="">
    </x-header>
  ...
  methods: {
    ...
    handlePay (id) {
      apiHttp.underCardPay(this.$route.query.spid, 'save', id).then(res => {
        if (res.code === 0) {
          this.jumpTo('/mall/cardPayment?spidOrder=' + res.data + '&spid=' + this.$route.query.spid)
