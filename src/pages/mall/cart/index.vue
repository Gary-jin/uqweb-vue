<template>
      <div class="cart">
        <x-header :left-options="{showBack: false}" class="cart-head" :style="{paddingTop: statusH + 'rem', height: statusH+0.88+'rem'}">
        <!--<x-header :left-options="{showBack: false}" class="cart-head">-->
        <!--<a @click="goBack()" slot="left" class="backBox"><i class="iconfont icon-back"></i></a>-->
          <a  class="head-right">购物车</a>
          <a slot="right" :class="{'head-right':true,'font-color':!showFoot}" @click="footShow" :style="{paddingTop: statusH + 'rem'}" v-if="list">管理</a>
        </x-header>
        <div class="cart-body" >
          <div class="mescrollBox" :style="{'bottom': list ? '50px' : '0'}">

            <mescroll-vue ref="mescroll" :down="mescrollDown" :up="mescrollUp" @init="mescrollInit">
              <div :style="{height: statusH + 'rem'}"></div>
              <div v-if="showEmpty">
                <div class="cart-item" v-for="(item,i) in list" :key="i">
                  <div class="item-head">
                    <span @click="selectItem('item',i)">
                      ...
                      <i class="iconfont icon-duihao2" style="color: #D92B24" v-if="item.isCheck"></i>
                    </span>
                    <!--<img :src="item.shopInfo ? item.shopInfo.logo : ''" alt="">-->
                    <!-- <img src="../../../assets/mall/img/cart/13.png" alt=""> -->
                    <div style="display:flex;align-items:center" @click="toShop(item.shopInfo.id)"> 
                  ...
                  <div class="item-body" v-for="(val,v) in item.goods" :key="v">
                    <div class="left" @click="selectItem('val',i,v)">
                      ...
                      <i class="iconfont icon-duihao2" style="color: #D92B24" v-if="val.isCheck"></i>
                    </div>
                    <div class="center1" @click="toGoodsDetails(val.goodsId)">
                      <img v-lazy="val.thumb" alt="">
                    </div>
                    <div class="right">
                      <p @click="toGoodsDetails(val.goodsId)">{{val.goodsName}}</p>
                      <p class="spec"><span>{{val.goodsField}}</span></p>
                      <div>
                        <span><i>￥</i>{{val.goodsPrice}}</span>
                        <div class="select-num">
                            <i class="minus" @click="reduce('update',val.id,val.goodsNum)">－</i>
                            <span>{{val.goodsNum}}</span>
                            <i class="add" @click="add('update',val.id,val.goodsNum)">＋</i>
                        </div>
                ...
                <div v-if="!list" class="cart-null">
                  <img src="../../../assets/mall/img/cartEmpty.png" alt="">
                  <p>购物车竟然空空如也~</p>
                  <button @click="vueJumpTo('/mall/home')">去逛逛</button>
                </div>
              </div>
              <div class="hot" v-if="likeGoodsList.length > 0">
                <img src="../../../assets/mall/img/cart/youLike.png" alt="">
              </div>
              <List :list="likeGoodsList"></List>
            </mescroll-vue>
          </div>
        </div>
        <!-- <div class="cart-foot" v-if="showFoot && list"> -->
          <div class="cart-foot" :style="{marginBottom: bot + 'px'}">
            <div class="left" @click="selectItem('all','','',list)">
              <i class="iconfont icon-circle2yuanquan" v-if="!selectAll"></i>
              <i class="iconfont icon-duihao2" style="color: #D92B24" v-if="selectAll"></i>
              <span>全选</span>
            </div>
            <div class="right">
              <div class="price">
                  <div class="all"><span>总计:</span><span><i>￥</i><span>{{price}}</span></span></div>
                  <span class="note">不含运费</span>
              </div>
              
              <div class="buy" @click="jumpTo('/mall/firmOrder?type='+'cart')">结算({{obj.goodsNum}})</div>
            </div>
        </div>
        <div class="cart-foot" v-if="!showFoot && list" :style="{marginBottom: bot + 'px'}">
          ...
          <div class="right">
            <div class="buy" @click="send('delete')">删除</div>
          </div>
        </div>
        <!-- <img src="../../../assets/mall/shouye.png" v-if="$store.state.global.firstNav === '/home'" alt="" class="homeBack" @click="goNativeHome()">         -->
        <div class="homeBack" id="dragDiv2"><img src="../../../assets/mall/shouye.png" @click="goNativeHome()"></div>
      </div>
</template>
...
import List from '../../../components/LikeList'
...
export default {
  ...
  methods: {
    ...
    upCallback (page, mescroll) {
      apiHttp.indexLikeList('1', page.num).then(res => {
        if (res.code === 0) {
          let arr = res.data.list === '' ? [] : res.data.list
          if (page.num === 1) this.shopList = []
          this.likeGoodsList = this.likeGoodsList.concat(arr)
          this.$nextTick(() => {
    ...
    reduce(type, id, num){
      num = num - 1
      if(num <= 0){
        this.$vux.toast.text('不能再少了哦~')
        return
      }
      this.send(type, id, num)
    },
    add(type, id, num){
      num = num + 1
      this.send(type, id, num)
