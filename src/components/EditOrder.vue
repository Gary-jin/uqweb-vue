    ...
    <div class="EditOrder">
      <div class="order">
        <div class="order-title">
          <p>{{data.title}}</p>
          <span v-if="data.right" @click="jumpLink(data.right_url)">{{data.right}}<img src="../assets/mall/img/my/left.png" alt=""></span>
        </div>
        ...
        <div class="list" v-show="expressList.length>0">
          <swiper :options="swiperOption4" ref="mySwiper1">
            <swiperSlide v-for="(item,index) in expressList" :key="index">
              <!-- <div class="list-text" @click="jumpTo('/mall/logistics?orderSn='+item.orderSn)"> -->
              <div class="list-text" @click="tologistics(item.orderSn)">
                ...
                <div>
                  <div>
                    ...
                    <span style="color: #fc3357;font-size: 0.3rem;width: 50%;" v-if="item.orderStatus == 2">已完成</span>
...
export default {
  ...
  methods: {
    tologistics(orderSn){
      ...
      // dsBridge.call("open",{url : 'http://unative.bangyaosoft.cn'+'/uqWeb/html/bargain/order_course.html?orderSn='+orderSn, type: 'web'})
