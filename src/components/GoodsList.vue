      ...
      <div :class="{goods_list_2 : !styles, goods_list_1 : styles}">
        ...
        <ul class="otherList">
          <li class="goods_li" v-for="item of list" :key="item.origin_id" @click="detail(item)">
            <div class="preview">
              <!--<img v-lazy="item.thumb" alt="" :key="item.thumb">-->
              <img :src="item.thumb" alt="" :key="item.thumb">
              <img src="../assets/img/home/video.png" alt="" class="video" v-if="item.video_url && showVideo"  @click.stop="openVideo(item)">
              <!-- <p class="commission_money" v-if="item.commission_money && item.level_commission_money > 0">升级赚￥{{item.level_commission_money}}</p> -->
            </div>
            <div class="otherInfo">
              <p class="brand">
                <img src="../assets/img/home/tblogo.png" alt="" v-if="Number(item.type) === 11">
              ...
              <!-- <p class="source">{{item.shop.title}}</p> -->
              <div class="up_gain" v-if="item.commission_money && item.commission_money != 0">
                <div class="coupon"><p class="dian"></p>{{item.coupon_money || 0}}元券</div>
                <span class="zhuan" v-if="item.commission_money">预估赚:￥{{item.commission_money}}</span>
              </div>
              <div class="original"><p class="newP"><span>￥</span>{{item.coupon_price}}<del>￥{{item.zk_final_price}}</del></p><p>已售：{{item.volume}}</p></div>
            </div>
          </li>
        </ul>
        <div id="empty"></div>
        ...
        .otherInfo{
          ...
          .up_gain{
            ...
            align-items: center;
              ...
              min-width: 0.7rem;
              // margin-right: .21rem;
              background:url("../assets/img/home/quan.png") no-repeat 0 0;
            ...
            .zhuan{
              ...
              box-shadow:1px 0.02rem 0.06rem 0px rgba(251,205,116,0.2);
